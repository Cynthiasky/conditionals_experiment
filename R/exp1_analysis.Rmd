...
```{r}
min_rt <- read.csv(file="../data/min_rt.csv", header=TRUE,sep=",")
min_rt <- min_rt[min_rt$experiment=='main',c('condition','pA','pB','pos','structure','response','expect_ans','responseTime')]
```

## visualize the distribution of RT
```{r}
...
#mu
ggplot(min_rt, aes(x=responseTime/1000, fill=condition, color=condition)) +
  ...
  geom_text(data=mu,aes(x=Mean/1000+0.3,y=5,label=round(Mean/1000,digits=3)),show.legend = FALSE) +
  labs(y="count", title = "Distribution of RTs (minimal)")
```

```{r}
summary(min_rt[min_rt$condition=="if","responseTime"])
summary(min_rt[min_rt$condition=="because","responseTime"])
```

## dataset exclude participants with technical issues
```{r}
data_all <- read.csv(file="../data/exp1_prolific_all.csv", header=TRUE,sep=",")
...
data_all<-data_all[!(data_all$submission_id==3533 | data_all$submission_id==3532 | data_all$submission_id==3530 | data_all$submission_id==3521),]
...
individual_count %>%
  filter(expect == TRUE) %>%
  ggplot(aes(x=structure, y=n, fill=condition,group=condition)) + 
  geom_bar(position="dodge", stat="identity") +
...
## if trials (conditionals) disjunctive/conjunctive
```{r}
cond_brm_out <- brm(formula = normal ~ structure+(1|submission_id), data = cond_disj_conj %>% filter(!submission_id %in% final_out$submission_id) %>% mutate(normal = ifelse(structure == "disjunctive",response == expect_ans,response != expect_ans)), family = bernoulli)

summary(cond_brm_out)
```
```{r}
brms::hypothesis(cond_brm_out, "structuredisjunctive > 0")
```

## because trials (fillers) disjunctive/conjunctive
```{r}
filler_brm_out <- brm(formula = normal ~ structure+(1|submission_id), data = filler_disj_conj %>% filter(!submission_id %in% final_out$submission_id)%>% mutate(normal = ifelse(structure == "disjunctive",response == expect_ans,response != expect_ans)), family = bernoulli)
```
```{r}
summary(filler_brm_out)
```

```{r}
brms::hypothesis(filler_brm_out, "structuredisjunctive > 0")
#brms::hypothesis(filler_brm_out, "Intercept < 0")
```
```{r}
...
exp_plot2 = ggplot(data = data_plot,
...
```{r}
all_brm_out <- brm(formula = normal~1+ structure*condition+(1+structure+condition|submission_id), data = data_disj_conj %>% mutate(normal = ifelse(structure == "disjunctive",response == expect_ans,response != expect_ans)), family = bernoulli)
```
```{r}
summary(all_brm_out)
```
...
compare_groups(model = all_brm_out, 
               higher = condition=='because' & structure=='disjunctive',
               lower = condition=='because' & structure=='conjunctive')
```


```{r}
compare_groups(model = all_brm_out, 
               higher = condition=='if' & structure=='disjunctive',
               lower = condition=='if' & structure=='conjunctive')
```
```{r}
compare_groups(model = all_brm_out, 
               higher = condition=='if' & structure=='disjunctive',
               lower = condition=='because' & structure=='disjunctive')
```
```{r}
